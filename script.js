function explore() {
    ...
    rabbitHoles.forEach(hole => {
        output += `
            <p class="hole" onclick="searchWeb('${hole}')">
                🕳️ ${hole}
            </p>
        `;
    });
    ...
    output += `
        ...
        <button onclick="searchReddit('${input}')">
            Reddit
        </button>

        <button onclick="searchWeb('${input}')">
            Web
        </button>

        <button onclick="searchArchive('${input}')">
            Archive
        </button>
...
function detectType(input) {
    ...
    if (input.startsWith("@")) {
        return "Username / Online Identity";
    }
...
function searchReddit(query) {
    window.open(
        `https://www.google.com/search?q=site:reddit.com+${encodeURIComponent(query)}`,
        "_blank"
    );
}
...
function searchArchive(query) {
    window.open(
        `https://web.archive.org/web/*/${encodeURIComponent(query)}`,
        "_blank"
    );
}
